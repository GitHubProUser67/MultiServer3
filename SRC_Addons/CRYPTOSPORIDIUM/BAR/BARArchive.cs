...
namespace PSMultiServer.SRC_Addons.CRYPTOSPORIDIUM.BAR
{
	...
	public class BARArchive
	{
		...
		public event BARArchive.BAREventDelegate OnLoadComplete;

		public event BARArchive.BARMessageDelegate OnMessage;

		public event BARArchive.BARLoadProgressEventDelegate OnLoadProgress;

		public event BARArchive.BARLoadHeaderEventDelegate OnLoadHeader;

		public event BARArchive.BAREventDelegate OnExtractComplete;

		public event BARArchive.BAREventDelegate OnRecompressComplete;
		...
		private void LoadHeaderandTOC()
		{
			...
			try
			{
				...
				foreach (object obj in m_toc)
				{
					...
					byte[] array;
					if ((ulong)tocentry.CompressedSize <= 4194304UL)
					{
					...
					else
					{
						...
						{
							int length = (int)Math.Min(4194304L, (long)((ulong)tocentry.CompressedSize - (ulong)num5));
							array2 = endianAwareBinaryReader3.ReadBytes(length);
							Array.Copy(array2, 0L, array, num5, (long)array2.Length);
						}
					}
        ...
        private void WriteMapAs(string mapFileName)
		{
			using (TextWriter textWriter = File.CreateText(mapFileName))
			{
				...
				num += 4U;
				textWriter.WriteLine("{0:X8} Id: {1}", num, m_header.Id);
				num += 4U;
				...
				textWriter.WriteLine("\n== Stats ==");
				float num5 = (num2 != 0L) ? ((1f - (float)num3 / (float)num2) * 100f) : 0f;
		...
		private void ReadHeader(Stream inStream)
		{
			...
			{
				throw new BARLoadException("File is not a BAR File or the header is corrupt", m_sourceFile);
			}
			...
			{
				throw new BARLoadException("Invalid BAR File Version or you need a newer version of the BAR Library", m_sourceFile);
			}
			m_header.Priority = endianAwareBinaryReader.ReadInt32();
			m_header.Id = endianAwareBinaryReader.ReadInt32();
		...
		private void ReadTOC(Stream inStream, EndianType endian)
		{
			...
			m_toc.Clear();
			int num = 0;
			while ((long)num < (long)((ulong)m_header.NumFiles))
			{
		...
		public void ReplaceFile(string filePath, BARAddFileOptions options)
		{
			...
			bool compress = BARArchive.ShouldCompress(filePath, options);
		...
		public void LoadAsync()
		{
			...
			BARArchive.LoadBARDelegate loadBARDelegate = new BARArchive.LoadBARDelegate(LoadInternal);
			try
			{
		...
		{
			BARArchive.LoadBARDelegate loadBARDelegate = (BARArchive.LoadBARDelegate)ar.AsyncState;
			try
			{
		...
		public void SaveAs(string fileName)
		{
			...
			if (flag)
			{
				...
				uint num = (uint)Utils.GetFourByteAligned((long)array2.Length);
		...
		public void Recompress(HashedFileName[] fileNames, CompressionMethod newMethod, ArchiveFlags newFlags)
		{
			foreach (HashedFileName hashedFileName in fileNames)
			{
				...
				else if (newMethod != CompressionMethod.Uncompressed && !BARArchive.ShouldCompress(tocentry.Path, BARAddFileOptions.Default))
				{
					ProgressMessage("Skipped " + hashedFileName, new object[0]);
				}
				else
		...
		{
			BeginUpdate();
			BARArchive.BarRecompressDelegate barRecompressDelegate = new BARArchive.BarRecompressDelegate(Recompress);
			try
			{
		...
		{
			BARArchive.BarRecompressDelegate barRecompressDelegate = (BARArchive.BarRecompressDelegate)ar.AsyncState;
			try
			{
		...
		{
			BeginUpdate();
			BARArchive.BarExtractAllDelegate barExtractAllDelegate = new BARArchive.BarExtractAllDelegate(ExtractAll);
		...
		{
			BARArchive.BarExtractAllDelegate barExtractAllDelegate = (BARArchive.BarExtractAllDelegate)ar.AsyncState;
			try
			{
