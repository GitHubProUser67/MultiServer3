...
namespace HTTPSecureServerLite.API.VEEMEE.nml
{
    public class Profile
    {
        ...
        public static string? Get(byte[]? PostData, string? ContentType)
        {
            ...
            if (PostData != null && ContentType == "application/x-www-form-urlencoded")
            {
                using (MemoryStream ms = new(PostData))
                {
                    ...
                    Directory.CreateDirectory($"{HTTPSServerConfiguration.APIStaticFolder}/VEEMEE/nml/User_Data");

                    string xmlProfile = string.Empty;

                    if (File.Exists($"{HTTPSServerConfiguration.APIStaticFolder}/VEEMEE/nml/User_Data/{psnid}.xml"))
                    {

                        // Load the XML string into an XmlDocument
                        XmlDocument xmlDoc = new();
                        xmlDoc.LoadXml($"{File.ReadAllText($"{HTTPSServerConfiguration.APIStaticFolder}/VEEMEE/nml/User_Data/{psnid}.xml")}");
                    ...
                    else
                    {
                        ...
                        File.WriteAllText($"{HTTPSServerConfiguration.APIStaticFolder}/VEEMEE/nml/User_Data/{psnid}.xml", XmlData);
        ...
        public static string? Set(byte[]? PostData, string? ContentType)
        {
            ...
            if (ContentType == "application/x-www-form-urlencoded" && PostData != null)
            {
                ...
                string apiPath = $"{HTTPSServerConfiguration.APIStaticFolder}/VEEMEE/nml/User_Data" + $"/{psnid}.xml";
                ...
                if (File.Exists(apiPath))
                {
                    ...
                    XDocument xmlDoc = XDocument.Parse($"{File.ReadAllText(apiPath)}");
                ...
                else
                {
                    ...
                    string outputPath = apiPath + $"/{psnid}.xml";
