        ...
        : ECCurve
    {
            ...
            : base(FiniteFields.GetPrimeField(q))
        {
            if (!isInternal)
            {
                ...
                if (unknownQ)
                {
                    int maxBitLength = ImplGetInteger("MultiServer.Addons.Org.BouncyCastle.EC.Fp_MaxSize", 1042); // 2 * 521
                    int certainty = ImplGetInteger("MultiServer.Addons.Org.BouncyCastle.EC.Fp_Certainty", 100);
