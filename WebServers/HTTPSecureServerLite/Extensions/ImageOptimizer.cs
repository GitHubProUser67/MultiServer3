                ...
                if (!string.IsNullOrEmpty(convertFileName))
                {
                    ...
                    try
                    {
                        ...
                        if (exitCode.HasValue && exitCode.Value == 0)
                        {
                            ...
                            LoggerAccessor.LogWarn($"[HTTPS] - ImageMagick conversion process failed with status code: {exitCode}");
                    }
                    catch (Exception ex)
                    {
                        LoggerAccessor.LogWarn($"[HTTPS] - ImageMagick conversion process failed - {ex}");
                    }
                    finally
                    {
                    ...
                    LoggerAccessor.LogWarn("[HTTPS] - Could not find ImageMagick for current architecture, aborting convert process.");
            }
            catch (Exception ex)
            {
                LoggerAccessor.LogError($"[HTTPS] - ImageUpscaler failed - {ex}");
            }
