...
using CryptoSporidium.Horizon.LIBRARY.Database.Config;
using CryptoSporidium.Horizon.LIBRARY.Database.Models;
using System.Text;
using System.Web;

namespace CryptoSporidium.Horizon.LIBRARY.Database
{
       ...
       */
        ...
        public async Task<LocationDTO[]?> GetLocations(int appId)
        {
            ...
            try
            {
                if (_settings.SimulatedMode)
                {
                    if (appId == 24000 || appId == 24180 || appId == 10680 || appId == 10683 || appId == 10684)
                    {
                        return new LocationDTO[]
                        {
                            new LocationDTO()
        ...
        public async Task SetServerSettings(int appId, Dictionary<string, string> settings)
        {
            try
            {
                if (_settings.SimulatedMode)
                {
                    Array.Resize(ref SimulatedAppIdList, SimulatedAppIdList.Length + 1);
                    SimulatedAppIdList[SimulatedAppIdList.Length - 1] = appId;
                }
                else
