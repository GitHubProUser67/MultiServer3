...
namespace PSMultiServer.PoodleHTTP.Addons.PlayStationHome.SSFW
{
    public class SSFWLayoutService
    {
        ...
        public static async Task HandleLayoutServiceGET(string directorypath, string absolutepath, HttpListenerRequest request, HttpListenerResponse response)
        {
            string inputurlfortrim = absolutepath;
            string[] words = inputurlfortrim.Split('/');
            ...
            if (inputurlfortrim != absolutepath) // If ends with UUID Ok.
            {
                ...
                string stringlayout = await SSFWProcessor.SSFWgetfurniturelayout(directorypath + "/mylayout.json", inputurlfortrim);
