...
namespace PSMultiServer.PoodleHTTP.Addons.PlayStationHome.SSFW
{
    ...
    {
        public static async Task HandleLogin(HttpListenerRequest request, HttpListenerResponse response)
        {
            int logoncount = 1;
            string sessionid = "";
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                if (File.Exists(userprofilefile))
                {
                    ...
                    SSFWUserData userData = JsonConvert.DeserializeObject<SSFWUserData>(Encoding.UTF8.GetString(await FileHelper.CryptoReadAsync(userprofilefile, SSFWPrivateKey.SSFWPrivatekey)));
                    ...
                    if (userData != null)
                    {
                        ...
                        await FileHelper.CryptoWriteAsync(Directory.GetCurrentDirectory() + $"{ServerConfiguration.SSFWStaticFolder}SSFW_Accounts/{sessionid}.json", SSFWPrivateKey.SSFWPrivatekey, Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(userData)));
                    }
                }
                else
                {
                ...
                if (!File.Exists(Directory.GetCurrentDirectory() + $"{ServerConfiguration.SSFWStaticFolder}RewardsService/cprod/rewards/" + resultString + "/mini.json"))
                    await FileHelper.CryptoWriteAsync(Directory.GetCurrentDirectory() + $"{ServerConfiguration.SSFWStaticFolder}RewardsService/cprod/rewards/" + resultString + "/mini.json", SSFWPrivateKey.SSFWPrivatekey, Encoding.UTF8.GetBytes(ServerConfiguration.SSFWMinibase));
                if (!File.Exists(Directory.GetCurrentDirectory() + $"{ServerConfiguration.SSFWStaticFolder}RewardsService/trunks-cprod/trunks/" + resultString + ".json"))
                    await FileHelper.CryptoWriteAsync(Directory.GetCurrentDirectory() + $"{ServerConfiguration.SSFWStaticFolder}RewardsService/trunks-cprod/trunks/" + resultString + ".json", SSFWPrivateKey.SSFWPrivatekey, Encoding.UTF8.GetBytes($"{{\"objects\":[]}}"));
                if (!File.Exists(Directory.GetCurrentDirectory() + $"{ServerConfiguration.SSFWStaticFolder}AvatarLayoutService/cprod/" + resultString + "/list.json"))
                    await FileHelper.CryptoWriteAsync(Directory.GetCurrentDirectory() + $"{ServerConfiguration.SSFWStaticFolder}AvatarLayoutService/cprod/" + resultString + "/list.json", SSFWPrivateKey.SSFWPrivatekey, Encoding.UTF8.GetBytes("[]"));
