namespace WatsonWebserver.Lite
{
    ...
    public class WebserverLite : WebserverBase, IDisposable
    {
        ...
        private async void ClientConnected(object sender, ClientConnectedEventArgs args)
        {
            ...
            TcpClient tcpClient = args.Client.Client;

            try
            {
                #region Retrieve-Headers

                using (Stream clientStream = _TcpServer.GetStream(args.Client.Guid))
                {
                    while (tcpClient.IsConnected())
                    {
                        if (tcpClient.Available > 0 && clientStream.CanWrite)
                        {
                            string httpIdent = clientStream.Readline();

                            if (httpIdent.Contains("HTTP/"))
                            {
                                const string newLineHttp = "\r\n";

                                sb.Clear();

                                sb.Append(httpIdent + newLineHttp);
                                ...
                                while ((line = clientStream.Readline()) != null)
                                {
                                    ...
                                    else if (sb.Length >= Settings.IO.MaxIncomingHeadersSize)
                                    {
