        ...
        public static string? ProcessVisitCounter2(byte[]? PostData, string? ContentType, string apipath)
        {
            ...
            if (!string.IsNullOrEmpty(boundary) && PostData != null)
            {
                ...
                if (key == ExpectedHash)
                {
                    ...
                    string Hash = VariousUtils.ByteArrayToHexString(MD5.HashData(Array.Empty<byte>())); // Seems to not make a difference.
            ...
            if (!string.IsNullOrEmpty(boundary) && PostData != null)
            {
                ...
                switch (func)
                {
                    case "submit":
                        ...
                        {
                            int best = 0;
                            string Hash = VariousUtils.ByteArrayToHexString(MD5.HashData(Array.Empty<byte>()));
                ...
                if (key == ExpectedHash)
                {
                    ...
                    switch (func)
                    {
                        ...
                        case "AddTicket":
                            ...
                            if (VariousUtils.IsBase64String(ticket))
                            {
