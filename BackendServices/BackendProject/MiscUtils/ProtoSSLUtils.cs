...
namespace BackendProject.ProtoSSLUtils;
...
public class ProtoSSLUtils
{
    ...
    public (AsymmetricKeyParameter, Certificate) GetGosEaCert()
    {
        string SubjectDN = "C=US, ST=California, O=\"Electronic Arts, Inc.\", OU=Online Technology Group, CN=gos.ea.com, emailAddress=fesl@ea.com";
        string cacheKey = "gos.ea.com";
    ...
    public (AsymmetricKeyParameter, Certificate) GetCustomEaCert(string CN)
    {
        string SubjectDN = $"C=US, ST=California, O=\"Electronic Arts, Inc.\", OU=Online Technology Group, CN={CN}, emailAddress=fesl@ea.com";

        if (_certCache.TryGetValue(CN, out var cacheHit))
        {
            // !TODO: New connections will break after running 10 years without a restart 
            return cacheHit;
        }
    ...
    private static X509Certificate PatchCertificateSignaturePattern(X509Certificate cCertificate)
    {
        ...
        var signature1Offset = VariousUtils.FindBytePattern(certDer, Sha1CipherSignature.Span);
        var signature2Offset = VariousUtils.FindBytePattern(certDer, Sha1CipherSignature.Span, signature1Offset + Sha1CipherSignature.Length);
