...
namespace HTTPSecureServer.API.OHS
{
    ...
    {
        public static string? Leaderboard_RequestByUsers(byte[] PostData, string ContentType, string directorypath, string batchparams)
        {
            ...
            if (string.IsNullOrEmpty(batchparams))
            {
                ...
                if (boundary != null)
                {
                    using (MemoryStream ms = new(PostData))
                    {
                        ...
                        dataforohs = RequestByUsers(CryptoSporidium.OHS.JaminProcessor.JaminDeFormat(data.GetParameterValue("data"), true), directorypath);
                        ms.Flush();
                    }
                }
            }
        ...
        public static string? Leaderboard_RequestByRank(byte[] PostData, string ContentType, string directorypath, string batchparams)
        {
            string? dataforohs = null;
            ...
            if (string.IsNullOrEmpty(batchparams))
            {
                ...
                if (boundary != null)
                {
                    using (MemoryStream ms = new(PostData))
                    {
                        var data = MultipartFormDataParser.Parse(ms, boundary);
                        LoggerAccessor.LogInfo($"[OHS] : Client Version - {data.GetParameterValue("version")}");
                        dataforohs = RequestByRank(CryptoSporidium.OHS.JaminProcessor.JaminDeFormat(data.GetParameterValue("data"), true), directorypath);
                        ms.Flush();
                    }
                }
            }
        ...
        public static string? Leaderboard_Update(byte[] PostData, string ContentType, string directorypath, string batchparams)
        {
            string? dataforohs = null;
        ...
        public static string? Leaderboard_UpdatesSameEntry(byte[] PostData, string ContentType, string directorypath, string batchparams)
        {
            string? dataforohs = null;
            string writekey = "11111111";
