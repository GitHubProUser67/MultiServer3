...
namespace SRVEmu.DirtySocks.DataStore
{
    public class DirtySocksJSONDatabase : IDatabase
    {
        ...
        public DirtySocksJSONDatabase()
        {
            ...
            try
            {
                if (File.Exists(eaEmuServerConfiguration.DirtySocksDatabaseConfig))
                {
                    using (FileStream file = File.Open(eaEmuServerConfiguration.DirtySocksDatabaseConfig, FileMode.Open, FileAccess.Read, FileShare.None))
                    {
        ...
        private void Save()
        {
            ...
            Directory.CreateDirectory(Path.GetDirectoryName(eaEmuServerConfiguration.DirtySocksDatabaseConfig));
            using (FileStream file = File.Open(eaEmuServerConfiguration.DirtySocksDatabaseConfig, FileMode.Create, FileAccess.Write, FileShare.None))
            {
