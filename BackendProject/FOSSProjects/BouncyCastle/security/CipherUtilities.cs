...
namespace Org.BouncyCastle.Security
{
    ...
    public static class CipherUtilities
    {
        ...
        private enum CipherMode { ECB, NONE, CBC, CCM, CFB, CTR, ENDIANCTR, CTS, EAX, GCM, GOFB, OCB, OFB, OPENPGPCFB, SIC, ENDIANSIC };
            ...
            if (parts.Length > 1)
            {
                ...
                try
                {
                    ...
                    case CipherMode.CTR:
                        ...
                        break;
                    case CipherMode.ENDIANCTR:
                        blockCipherMode = new EndianSicBlockCipher(blockCipher);
                    break;
                        case CipherMode.CTS:
                        cts = true;
                        blockCipherMode = new CbcBlockCipher(blockCipher);
                    ...
                    case CipherMode.SIC:
                        ...
                        blockCipherMode = new SicBlockCipher(blockCipher);
                        break;
                    case CipherMode.ENDIANSIC:
                        if (blockCipher.GetBlockSize() < 16)
                        {
                            throw new ArgumentException("Warning: ENDIAN SIC-Mode can become a twotime-pad if the blocksize of the cipher is too small. Use a cipher with a block size of at least 128 bits (e.g. AES)");
                        }
                        blockCipherMode = new EndianSicBlockCipher(blockCipher);
                        break;
                        default:
