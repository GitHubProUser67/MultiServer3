    ...
    public static class VEEMEEProfileManager
    {
        public static async Task<byte[]> ReadProfile(string psnid, string product, string hex, string salt)
        {
            ...
            if (File.Exists(Directory.GetCurrentDirectory() + $"{ServerConfiguration.HTTPStaticFolder}HOME_VEEMEE/Acorn_Medow/User_Profiles/{psnid}.json"))
                reader = Encoding.UTF8.GetString(FileHelper.CryptoReadAsync(Directory.GetCurrentDirectory() + $"{ServerConfiguration.HTTPStaticFolder}HOME_VEEMEE/Acorn_Medow/User_Profiles/{psnid}.json", HTTPPrivateKey.HTTPPrivatekey));
            else
                reader = Encoding.UTF8.GetString(FileHelper.CryptoReadAsync(Directory.GetCurrentDirectory() + $"{ServerConfiguration.HTTPStaticFolder}HOME_VEEMEE/Acorn_Medow/default_profile.json", HTTPPrivateKey.HTTPPrivatekey));
        ...
        {
            await FileHelper.CryptoWriteAsync(Directory.GetCurrentDirectory() + $"{ServerConfiguration.HTTPStaticFolder}HOME_VEEMEE/Acorn_Medow/User_Profiles/{psnid}.json", HTTPPrivateKey.HTTPPrivatekey, Encoding.UTF8.GetBytes(profile), true);
