            ...
            foreach (var command in commands)
            {
                ...
                switch (method)
                {
                    ...
                    case "usercounter/set/":
                        ...
                        resultfromcommand = await OHSUserCounter.set(directorypath + $"/{project}/", data, request, response);

                        break;

                    case "usercounter/getall/":

                        resultfromcommand = await OHSUserCounter.get_all(directorypath + $"/{project}/", data, request, response);

                        break;

                    case "usercounter/get/":

                        resultfromcommand = await OHSUserCounter.get(directorypath + $"/{project}/", data, request, response);

                        break;

                    case "userinventory/getglobalitems/":

                        resultfromcommand = await OHSUserInventory.getGlobalItems(directorypath + $"/{project}/", data, request, response);

                        break;

                    case "userinventory/getuserinventory/":

                        resultfromcommand = await OHSUserInventory.getUserInventory(directorypath + $"/{project}/", data, request, response);
