        ...
        public static async Task<string> set(string directorypath, string batchparams, bool global, HttpListenerRequest request, HttpListenerResponse response)
        {
            ...
            if (!global)
            {
                ...
                if (File.Exists(profiledatastring))
                {
                    ...
                    if (profiledata != null)
                    {
                        jObject = JObject.Parse(profiledata);
            ...
            else
            {
                ...
                if (File.Exists(globaldatastring))
                {
                    ...
                    if (globaldata != null)
                    {
                        jObject = JObject.Parse(globaldata);
                        ...
                        await FileHelper.CryptoWriteAsync(globaldatastring, HTTPPrivateKey.HTTPPrivatekey, Encoding.UTF8.GetBytes(jObject.ToString(Formatting.None)), false);
                    }
                }
                else
                {
                    ...
                    await FileHelper.CryptoWriteAsync(globaldatastring, HTTPPrivateKey.HTTPPrivatekey, Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(newProfile)), false);
                }
            }
