...
namespace MultiServer.HTTPService
{
    ...
    {
        public Task MapperStart(string foldertomap, string prefix, string bruteforce)
        {
            ...
            {
                if (bruteforce == "on")
                    CopyFiles(Directory.GetCurrentDirectory() + ServerConfiguration.HomeToolsHelperStaticFolder, foldertomap);

                IEnumerable<string> strings = Directory.EnumerateFiles(foldertomap, "*.*", SearchOption.AllDirectories).Where(s => s.EndsWith(".mdl") || s.EndsWith(".efx") || s.EndsWith(".xml") || s.EndsWith(".scene") || s.EndsWith(".map"));
                    ...
                    foreach (FileInfo file in new DirectoryInfo(foldertomap).GetFiles(str + ".*"))
                    {
                        if (File.Exists(Path.Combine(foldertomap, file.Name)))
                        {
                            ...
                            if (!File.Exists(Path.Combine(foldertomap, text.ToUpper())))
                            {
                                ...
                                ServerConfiguration.LogInfo($"[Mapper] - Mapped {file.Name} -> {text.ToUpper()}");
                            }
                        }
                    }
                        ...
                        foreach (FileInfo file in new DirectoryInfo(foldertomap).GetFiles(str2 + ".*"))
                        {
                            if (File.Exists(Path.Combine(foldertomap, file.Name)))
                            {
                                ...
                                if (!File.Exists(Path.Combine(foldertomap, cdatafromatmos.ToUpper())))
                                {
                                    ...
                                    ServerConfiguration.LogInfo($"[Mapper] - Mapped {file.Name} -> {cdatafromatmos.ToUpper()}");
                                }
                            }
                        }
                    }
            ...
            {
                ServerConfiguration.LogError($"[Mapper] - An Error happened in MapperStart - {ex}");
            }
        ...
        private void MapperPrepareFiles(string foldertomap)
        {
            ...
            {
                ServerConfiguration.LogError($"[Mapper] - An Error happened in MapperPrepareFiles - {ex}");
            }
        }
