...
namespace SRVEmu.DirtySocks.DataStore
{
    public class JSONDatabase : IDatabase
    {
        ...
        public JSONDatabase()
        {
            ...
            try
            {
                if (File.Exists(SRVEmuServerConfiguration.DirtySocksDatabaseConfig))
                {
                    using (FileStream file = File.Open(SRVEmuServerConfiguration.DirtySocksDatabaseConfig, FileMode.Open, FileAccess.Read, FileShare.None))
                    {
        ...
        private void Save()
        {
            ...
            Directory.CreateDirectory(Path.GetDirectoryName(SRVEmuServerConfiguration.DirtySocksDatabaseConfig));
            using (FileStream file = File.Open(SRVEmuServerConfiguration.DirtySocksDatabaseConfig, FileMode.Create, FileAccess.Write, FileShare.None))
            {
