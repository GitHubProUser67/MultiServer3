...
namespace SRVEmu.Blaze.BlazeUtils
{
    public class BlazeUtils
    {
        ...
        public static string TagToLabel(uint Tag)
        {
            ...
            List<byte> buff = new(BitConverter.GetBytes(Tag));
            buff.Reverse();
            byte[] res = new byte[4];
        ...
        public static byte[] PacketToRaw(Packet p)
        {
            ...
            res.AddRange(p.Content ?? Array.Empty<byte>());
            return res.ToArray();
        }
        ...
        {
            string text = ListToText(string.Empty, ReadPacketContent(p));
            return text;
        }
        ...
        {
            List<Tdf> res = new();
            MemoryStream m = new(p.Content ?? Array.Empty<byte>());
            m.Seek(0, 0);
            try
            {
        ...
        public static Dictionary<ushort, string> Components = new()
        {
        ...
        public static Dictionary<uint, string> Commands = new()
        {
        ...
        public static Dictionary<uint, string> Notifications = new()
        {
            // Game Manager Component
