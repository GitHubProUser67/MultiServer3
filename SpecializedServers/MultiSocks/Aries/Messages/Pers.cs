        ...
        public override void Process(AbstractAriesServer context, AriesClient client)
        {
            ...
            if (!string.IsNullOrEmpty(context.Project) && (context.Project.Contains("BURNOUT5") || context.Project.Contains("DPR-09")))
            {
               ...
               They expect it to arrive after newsnew8 because of the delay original server had, so we must "simulate" this delay. */
                user.SendPlusWho(user, client.VERS, !string.IsNullOrEmpty(context.Project) && context.Project.Contains("BURNOUT5"));
            }
            else
            {
                client.SendMessage(new Pers()
