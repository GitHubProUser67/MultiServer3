        ...
        public override void Process(AbstractAriesServerV1 context, AriesClient client)
        {
            ...
            if (int.TryParse(GetInputCacheValue("START"), out int start) && int.TryParse(GetInputCacheValue("COUNT"), out int count))
            {
                ...
                if (!string.IsNullOrEmpty(LANG) && LANG != "-1" 
                    ...
                    && context.Project.Equals("DPR-09")) // Hasbro Family game Night does custom filtering on top for region specific lobbies.
                    MatchingList = MatchingList.Where(game => game.Params.Contains($"LANG%3d{LANG}") 
                    && game.Params.Contains($"VER%3d{GetInputCacheValue("VER")}")).ToList();
