...
public class VulnerableCertificateGenerator
{
    ...
    public (AsymmetricKeyParameter, Certificate, X509Certificate2) GetVulnerableFeslEaCert()
    {
        ...
        (AsymmetricKeyParameter, Certificate, X509Certificate2) creds = GenerateVulnerableCert(IssuerDN, SubjectDN);
    ...
    public (AsymmetricKeyParameter, Certificate, X509Certificate2) GetVulnerableLegacyCustomEaCert(string CN, string email, bool WeakChainSignedRSAKey)
    {
        if (_certCache.TryGetValue(CN, out (AsymmetricKeyParameter, Certificate, X509Certificate2) cacheHit))
            // !TODO: New connections will break after running 10 years without a restart 
            return cacheHit;
    ...
    /// </summary>
    private static (AsymmetricKeyParameter, Certificate, X509Certificate2) GenerateVulnerableCert(string issuer, string subject)
    {
    ...
    /// Generates a vulnerable certificate for vulnerable ProtoSSL versions.
    /// </summary>
    private static (AsymmetricKeyParameter, Certificate, X509Certificate2) GenerateVulnerableLegacyCert(string RootCN, string Rootemail, string ChainCN, string Chainemail, bool WeakChainSignedRSAKey)
    {
        BcTlsCrypto crypto = new(new SecureRandom());
