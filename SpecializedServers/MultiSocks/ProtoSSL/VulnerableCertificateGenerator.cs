...
public class VulnerableCertificateGenerator
{
    ...
    public (AsymmetricKeyParameter, Certificate, X509Certificate2) GetVulnerableFeslEaCert(bool EnableExploit)
    {
        ...
        (AsymmetricKeyParameter, Certificate, X509Certificate2) creds = GenerateVulnerableCert(IssuerDN, SubjectDN, EnableExploit);
        _certCache.TryAdd(cacheKey, creds);
        return creds;
    }

    public (AsymmetricKeyParameter, Certificate, X509Certificate2) GetVulnerableCustomEaCert(string CN, string email, bool EnableExploit)
    {
    ...
    public (AsymmetricKeyParameter, Certificate, X509Certificate2) GetVulnerableLegacyCustomEaCert(string CN, string email, bool WeakChainSignedRSAKey, bool EnableExploit)
    {
        if (_certCache.TryGetValue(CN, out (AsymmetricKeyParameter, Certificate, X509Certificate2) cacheHit))
            // !TODO: New connections will break after running 10 years without a restart 
            return cacheHit;

        (AsymmetricKeyParameter, Certificate, X509Certificate2) creds = GenerateVulnerableLegacyCert("OTG3 Certificate Authority", "dirtysock-contact@ea.com",
            CN, email, WeakChainSignedRSAKey, EnableExploit);
        _certCache.TryAdd(CN, creds);
        return creds;
    }
    ...
    /// </summary>
    private static (AsymmetricKeyParameter, Certificate, X509Certificate2) GenerateVulnerableCert(string issuer, string subject, bool EnableExploit)
    {
        ...
        if (!EnableExploit)
        {
            ...
            certEntry = new(GenerateCertificate(SHA1CipherAlgorithm, subject,
        ...
        else
        {
            ...
            store = new Pkcs12StoreBuilder().Build();
            certEntry = new(PatchCertificateSignaturePattern(GenerateCertificate(SHA1CipherAlgorithm, subject,
            cKeyPair, caKeyPair.Private, GenerateCertificate(SHA1CipherAlgorithm, issuer, caKeyPair, caKeyPair.Private))));
    ...
    /// Generates a vulnerable certificate for vulnerable ProtoSSL versions.
    /// </summary>
    private static (AsymmetricKeyParameter, Certificate, X509Certificate2) GenerateVulnerableLegacyCert(string RootCN, string Rootemail, string ChainCN, string Chainemail, bool WeakChainSignedRSAKey, bool EnableExploit)
    {
        X509CertificateEntry certEntry;
        AsymmetricCipherKeyPair caKeyPair;
        Pkcs12Store store;
        ...
        if (!EnableExploit)
        {
            ...
            store = new Pkcs12StoreBuilder().Build();
            certEntry = new(GenerateLegacyCertificate(MD5CipherAlgorithm, ChainCN, Chainemail,
                cKeyPair, caKeyPair.Private, GenerateLegacyCertificate(MD5CipherAlgorithm, RootCN, Rootemail, caKeyPair, caKeyPair.Private)));
        ...
        else
        {
            ...
            store = new Pkcs12StoreBuilder().Build();
            certEntry = new(PatchCertificateSignaturePattern(GenerateLegacyCertificate(MD5CipherAlgorithm, ChainCN, Chainemail,
    ...
    private static Org.BouncyCastle.X509.X509Certificate GenerateLegacyCertificate(string CipherAlgorithm, string CN, string email, AsymmetricCipherKeyPair subjectKeyPair, AsymmetricKeyParameter issuerPrivKey, Org.BouncyCastle.X509.X509Certificate? issuerCert = null)
    {
        X509V3CertificateGenerator certGen = new();
        certGen.SetSerialNumber(BigIntegers.CreateRandomInRange(BigInteger.One, BigInteger.ValueOf(long.MaxValue), new SecureRandom()));
        ...
        certGen.SetNotAfter(new DateTime(2009, 05, 23)); // No choice, ProtoSSL cannot handle years after 2050 (client bug).
    ...
    private static Org.BouncyCastle.X509.X509Certificate ReadPemCertificate(string pathToPemFile)
    {
        return new X509CertificateParser().ReadCertificate(File.ReadAllBytes(pathToPemFile));
    }
