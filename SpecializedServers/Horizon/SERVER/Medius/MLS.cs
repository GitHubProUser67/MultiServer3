                                        ...
                                        switch (clientCheatQuery.StartAddress)
                                        {
                                            case 65536:
                                                if (data.ClientObject != null)
                                                {
                                                    ...
                                                    {
                                                        string anticheatMsg = $"[MLS] - HOME ANTI-CHEAT - DETECTED UNKNOWN EBOOT - User:{data.ClientObject.IP + ":" + data.ClientObject.AccountName} CID:{data.MachineId}";
                            ...
                            foreach (var friend in data.ClientObject.FriendsListPS3)
                            {
                                ...
                                friendListResponses.Add(new MediusGetBuddyList_ExtraInfoResponse()
                                {
                                    ...
                                    OnlineState = new MediusPlayerOnlineState()
                                    {
                                        ...
                                        MediusLobbyWorldID = friendClient?.CurrentChannel?.Id ?? 0,
                                        MediusGameWorldID = friendClient?.CurrentGame?.MediusWorldId ?? 0,
                ...
                case MediusLadderList_ExtraInfoRequest0 ladderList_ExtraInfoRequest0:
                    {
                        ...
                        await HorizonServerConfiguration.Database.GetLeaderboard(ladderList_ExtraInfoRequest0.LadderStatIndex + 1, (int)ladderList_ExtraInfoRequest0.StartPosition - 1, (int)ladderList_ExtraInfoRequest0.PageSize, data.ApplicationId).ContinueWith((r) =>
                        {
                            ...
                            if (r.IsCompletedSuccessfully && r.Result != null)
                            {
                                ...
                                {
                                    byte[] mediusStats = new byte[Constants.ACCOUNTSTATS_MAXLEN];
                                    try { var dbAccStats = Convert.FromBase64String(ladderEntry.MediusStats ?? string.Empty); mediusStats = dbAccStats; } catch (Exception) { }
                                    ...
                                    responses.Add(new MediusLadderList_ExtraInfoResponse()
                                    {
                                        ...
                                        OnlineState = new MediusPlayerOnlineState()
                                        {
                                            ...
                                            MediusGameWorldID = account?.CurrentGame?.MediusWorldId ?? 0,
                                            MediusLobbyWorldID = account?.CurrentChannel?.Id ?? 0
                                        },
                                        StatusCode = MediusCallbackStatus.MediusSuccess,
                                        EndOfList = false
                                    });
                ...
                case MediusFindPlayerRequest findPlayerRequest:
                    {
                        ...
                        else
                        {
                            ...
                            data.ClientObject.Queue(new MediusFindPlayerResponse()
                            {
                                ...
                                MediusWorldID = (foundPlayer.PlayerStatus == MediusPlayerStatus.MediusPlayerInGameWorld) ? foundPlayer.CurrentGame?.MediusWorldId ?? 0 : foundPlayer.CurrentChannel?.Id ?? 0,
                                EndOfList = true
                            });
                ...
                case MediusPlayerInfoRequest playerInfoRequest:
                    {
                        ...
                        else
                        {
                            await HorizonServerConfiguration.Database.GetAccountById(playerInfoRequest.AccountID).ContinueWith((r) =>
                            {
                                ...
                                {
                                    byte[] mediusStats = new byte[Constants.ACCOUNTSTATS_MAXLEN];
                                    try { var dbAccStats = Convert.FromBase64String(r.Result.MediusStats ?? string.Empty); mediusStats = dbAccStats; } catch { }
                            ...
                            .ContinueWith(r =>
                            {
                                ...
                                if (r.IsCompletedSuccessfully && r.Result != null)
                                {
                                    responses.AddRange(r.Result.ClanMember.Select(x =>
                                    {
                                        ...
                                        byte[] mediusStats = new byte[Constants.ACCOUNTSTATS_MAXLEN];
                                        try { var dbAccStats = Convert.FromBase64String(cachedAccount.Result.MediusStats ?? string.Empty); mediusStats = dbAccStats; } catch (Exception) { }
                ...
                case MediusReassignGameMediusWorldID reassignGameMediusWorldID:
                    {
                        ...
                        uint result = data.ClientObject.CurrentGame.ReassignGameMediusWorldID(reassignGameMediusWorldID);
                        if (result != 0)
                        {
                            uint iCurrentMediusWorldID = reassignGameMediusWorldID.OldMediusWorldID;
                            uint iNewMediusWorldID = result;
                        ...
                        if (data.ClientObject.CurrentGame != null)
                        {
                            if (data.ApplicationId == 20371 || data.ApplicationId == 20374)
                            {
                                ...
                                if (isHomeCheat)
                                {
                                    _ = Task.Run(() =>
                                    {
                                        Parallel.ForEach(homeAntiCheatPlugin.ProcessAntiCheatRequest(data.ClientObject, HomeUserEntry), requestParameters =>
                                        {
                                            CheatQuery(requestParameters.Item1, requestParameters.Item2, clientChannel, requestParameters.Item3, requestParameters.Item4);
                                        });
                                    });
                                }
                            }
                        ...
                        else
                        {
                            var results = channel.LocalClients.Where(x => x.IsConnected).Select(x => new MediusGetLobbyPlayerNames_ExtraInfoResponse()
                            {
                                ...
                                OnlineState = new MediusPlayerOnlineState()
                                {
                                    ...
                                    MediusGameWorldID = x.CurrentGame?.MediusWorldId ?? 0,
                                    MediusLobbyWorldID = x.CurrentChannel?.Id ?? 0
                                },
                                EndOfList = false
                            }).ToArray();
