...
using NAudio.Wave; // mp3/wav validation
...
namespace NautilusXP2024
{
    ...
    {
        private readonly string logFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "logs", "JobReport.txt");
        ...
        {
            // Retrieve files only from the root directory for unmapped file checks
            ...
            // Retrieve files from all directories for all other validations
                ...
                HashSet<string> allowedZeroByteFiles = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
{
    "BAGS.XML", "GAME_FLOW.LUA", "INITIALIZE.LUA", "EXPORTEDINST.LUA", "MAGICCIRCLE.LUA", "BULLET.LUA", "PHASEGAME.LUA"
                ...
                if (fileInfo.Length == 0 && !allowedZeroByteFiles.Contains(fileInfo.Name.ToUpper()))
                {
                    ...
                    continue; // Skip further validation for this zero byte file unless it's allowed to be zero bytes
                }
